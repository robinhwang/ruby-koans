class AboutRegularExpression < EdgeCase::Koan
  def test_a_pattern_is_a_regular_expression
    assert_equal Regexp, /pattern/.class
  end

  def test_a_regexp_can_search_a_string_for_matching_content
    assert_equal "match", "some macthing content"[/match/]
  end
  ...
  def test_character_classes_give_options_for_a_character
    ...
    assert_equal ["cat", "bat", "rat"], animals.select{ |a|
      a[/[cbr]at/]
    }
  end
  ...
  def test_slash_w_is_a_shortcut_for_a_word_character_class
    assert_equal "variable_1", "variable_1 = 42"[/[a-zA-Z0-9]+/]
  ...
  def test_parentheses_group_contents
    assert_equal "ahahaha", "ahahaha"[/(ha)+/]
  end

  def test_parantheses_also_capture_matched_content_by_number
    assert_equal "Gray", "Gray James"[/(\w+), (\w+)/, 1]
    assert_equal "James", "Gray, James"[/(\w+), (\w+)/, 2]
  end

  def test_variables_can_alse_be_used_to_access_captures
  ...
  def test_gsub_is_like_find_and_replace_all
    assert_equal "one t-t", "ine two-three".gsub(/(t\w*)/){
      $1[0, 1]
    }
  end
end
